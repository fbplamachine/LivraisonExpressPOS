...
public class ToValidateFragment extends Fragment {
    ...
    private void stream() {
        mOrderViewModel.getToValidateList().observe(getViewLifecycleOwner(), new Observer<List<OrderSteed>>() {
            @Override
            public void onChanged(List<OrderSteed> orderSteeds) {
                ...
                if (mOrderViewModel.getTodayPage() == 1){
                    Collections.reverse(steeds);
